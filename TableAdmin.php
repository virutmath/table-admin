...
class TableAdmin
{
	 ...
	 */
	public function setRowAttr($arrayField = []) {
		$this->rowAttr = $arrayField;
	}
	...
	private function tr($row_data, $record_id)
	{
		//set row attribute
		$rowAttr = '';
		if($this->rowAttr && is_array($this->rowAttr)) {
			foreach ($this->rowAttr as $attr=>$fieldName) {
				$rowAttr .= $attr . '="' . $this->prepareFieldName($row_data,$fieldName) . '" ';
			}
		}
		$str = '<tr id="tableAdmin-tr-' . $record_id . '" '.$rowAttr.'>';
